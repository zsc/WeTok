...
from src.WeTok.modules.diffusionmodules.improved_model import Encoder, Decoder, GANDecoder
from src.WeTok.modules.vqvae.lookup_free_quantize import LFQ, GFQ
...
class VQModel(L.LightningModule):
    def __init__(self,
        ...
        if not self.use_GFQ:
            ...
            self.quantize = GFQ(dim=embed_dim, codebook_size=n_embed, num_codebooks=num_codebooks,
                                sample_minimization_weight=sample_minimization_weight, 
                                batch_maximization_weight=batch_maximization_weight, 
                                token_factorization=token_factorization, factorized_bits=factorized_bits
                )
